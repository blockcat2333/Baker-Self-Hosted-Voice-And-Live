...
- Stable server releases use numeric tags such as `1.1.1`; server beta releases may use compact labels such as `1.1.1beta.1`. The matching Docker image is `blockcat233/baker:<version>`.
...
- Package metadata stays semver-compatible for tooling. Stable server labels such as `1.1.1` are stored directly in `package.json`; beta labels such as `1.1.1beta.1` are stored as `1.1.1-beta.1`.
