...
describe('update version discovery', () => {
  it('sorts semver tags descending and prefers stable tags over prereleases', () => {
    expect(['1.0.3', '1.1.0-beta.1', '1.1.0', '1.0.10'].sort(compareVersionTagsDesc)).toEqual([
      '1.1.0',
      '1.1.0-beta.1',
      '1.0.10',
      '1.0.3',
  ...
  it('filters Docker Hub tags and merges GitHub release metadata best-effort', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({
        json: async () => [
          {
            body: 'Release notes',
            html_url: 'https://github.com/blockcat2333/Baker-Self-Hosted-Voice-And-Live/releases/tag/v1.0.4',
            tag_name: 'v1.0.4',
          },
        ],
        ok: true,
    ...
    expect(versions.map((version) => version.tag)).toEqual(['1.0.4', '1.0.3']);
