...
for (const packagePath of [...listPackageJsons('apps'), ...listPackageJsons('packages')]) {
...
for (const docsPath of [
  ...
  const pinnedDockerTags = [...text.matchAll(/blockcat233\/baker:(?!latest\b)([A-Za-z0-9._-]+)/g)].map(
    (match) => match[1],
  );
