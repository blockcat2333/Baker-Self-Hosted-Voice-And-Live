...
if [ ! -f "$RUNTIME_ENV" ]; then
  ...
  tmp_env="${RUNTIME_ENV}.tmp"
  {
    ...
    write_runtime_kv MEDIA_REGION_PROFILES "$MEDIA_REGION_PROFILES"
    ...
    write_runtime_kv TURN_ENABLED "$TURN_ENABLED"
...
echo "Tip: mount /var/lib/baker for persistence, and use HTTPS for browser voice, camera, and screen sharing."
